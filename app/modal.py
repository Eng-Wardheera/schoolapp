...
class Class(db.Model):
    ...
    # Ardayda fasalkan ku jirta (Midkan ayaa kafiican class_student_list)
    # Markaad isticmaalayso: my_class.students
    students = db.relationship('Student', back_populates='class_rel', passive_deletes=True)
...
class TimeSlot(db.Model):
    ...
    # Basic Info
    label = db.Column(db.String(50), nullable=False) # Tusaale: "Xiisadda 1aad", "Break"
    start_time = db.Column(db.Time, nullable=False)   # Tusaale: 07:30:00
    end_time = db.Column(db.Time, nullable=False)     # Tusaale: 08:15:00
    
    # Ma xiisad baa mise waa nasasho?
    is_break = db.Column(db.Boolean, default=False) 
    
    # Shift-ka uu ka tirsan yahay (Morning/Afternoon)
    shift = db.Column(
        db.Enum('morning', 'afternoon', name='timeslot_shift_enum'), 
        nullable=False
    )
    ...
    # ✅ Ka hortagga in hal iskuul isku waqti laba Slot u sameeyo
    __table_args__ = (
        db.UniqueConstraint(
            'school_id', 'branch_id', 'start_time', 'shift', 
            name='uq_timeslot_per_branch'
        ),
    )

    # ================= RELATIONSHIPS (Old Style) =================
    school = db.relationship(
        'School', 
        backref=db.backref('time_slots', cascade='all, delete-orphan', passive_deletes=True)
    )
    branch = db.relationship(
        'Branch', 
        backref=db.backref('time_slots', passive_deletes=True)
    )

    def __repr__(self):
        type_slot = "Break" if self.is_break else "Period"
        return f"<{type_slot} {self.label}: {self.start_time}-{self.end_time}>"
...
class Timetable(db.Model):
    ...
    # Info
    day_of_week = db.Column(db.Enum('Saturday', 'Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', name='days_enum'), nullable=False)
    created_at = db.Column(db.DateTime, default=now_eat)
    updated_at = db.Column(db.DateTime, default=now_eat, onupdate=now_eat)

    # ✅ Ka hortagga isku-dhaca (Unique Constraints)
    __table_args__ = (
        # Macallinka hal waqti hal meel kaliya ha joogo (Conflict Prevention)
        db.UniqueConstraint('teacher_id', 'time_slot_id', 'day_of_week', name='uq_teacher_timing'),
        # Fasalka hal waqti hal maaddo ha u socoto (Conflict Prevention)
        db.UniqueConstraint('class_id', 'section_id', 'time_slot_id', 'day_of_week', name='uq_class_timing'),
    )

    # ================= RELATIONSHIPS (Corrected Backrefs) =================
    # MAGAC KASTA OO BACKREF AH WAA INUU NOQODAA MID GAAR AH (UNIQUE)
    
    school = db.relationship('School', backref=db.backref('school_timetables', cascade='all, delete-orphan', passive_deletes=True))
    branch = db.relationship('Branch', backref=db.backref('branch_timetables', passive_deletes=True))
    
    subject = db.relationship('Subject', backref=db.backref('subject_timetables', cascade='all, delete-orphan', passive_deletes=True))
    teacher = db.relationship('Teacher', backref=db.backref('teacher_timetables', cascade='all, delete-orphan', passive_deletes=True))
    
    klass = db.relationship('Class', backref=db.backref('class_timetables', cascade='all, delete-orphan', passive_deletes=True))
    section = db.relationship('Section', backref=db.backref('section_timetables', cascade='all, delete-orphan', passive_deletes=True))
    
    time_slot = db.relationship('TimeSlot', backref=db.backref('slot_timetables', cascade='all, delete-orphan', passive_deletes=True))


    def __repr__(self):
        return f"<Timetable {self.day_of_week} Slot:{self.time_slot_id}>"
...
class Student(db.Model):
    ...
    # HAL xiriir oo Class ah (Waxaan isku darnay student_class iyo class_obj)
    # Markaad isticmaalayso: student.class_rel.name
    ...
    # Xiriirka Natiijooyinka (results)
...
class StudentExamResult(db.Model):
    ...
    school = db.relationship('School', backref=db.backref('school_exam_results', overlaps="exam_results"))

    branch = db.relationship('Branch', backref=db.backref('branch_exam_results', overlaps="exam_results"))

    academic_year = db.relationship(
        'AcademicYear',
        backref=db.backref('year_exam_results', passive_deletes=True)
    )

    exam = db.relationship(
        'Exam',
        backref=db.backref('exam_results_list', passive_deletes=True)
    )

    # MUHIIM: Waxaan u beddelnay 'back_populates' si uu ula xiriiro Student.results
    # Waxaan ku darnay overlaps haddii meel kale 'exam_results' looga yeeray
    student = db.relationship(
        'Student',
        back_populates='results',
        overlaps="exam_results"
    )

    class_rel = db.relationship('Class', backref=db.backref('class_exam_results', overlaps="exam_results"))

    section = db.relationship('Section', backref=db.backref('section_exam_results', overlaps="exam_results"))
