...
import pytz
from flask import Blueprint, Response, after_this_request, current_app, flash, g, json, jsonify, make_response, render_template, request, redirect, send_file, session, url_for
...
def login():
    ...
    if form.validate_on_submit():
        ...
        # 🔥 CREATE UNIQUE SESSION TOKEN (VERY IMPORTANT)
        import uuid
        ...
        log = UserLog(
            ...
            interface_name=interface_name,
            extra_info=f"Manufacturer: {manufacturer}, Model: {model}",
            status="login",
            action="login"
        )
        db.session.add(log)
        ...
        # -----------------------------
        # ✅ Create UserSession (WITH TOKEN 🔥)
        session_entry = UserSession(
            id=uuid.uuid4().hex,
            user_id=user.id,
            session_token=session_token,   # 🔥 CRITICAL
            ip_address=ip_address,
        ...
        # 🔥 SAVE SESSION INFO (VERY IMPORTANT)
...
def google_authorize():
    try:
        ...
        # Hubi haddii user horey u diiwaan gashan yahay
        user = User.query.filter(
            (User.email == email) | (User.fullname == fullname)
        ).first()

        if user:
            # Haddii horey u jira, login
            login_user(user)
            flash(f"Welcome back, {user.fullname}!", "success")
        else:
            # Haddii user cusub yahay, abuuro account cusub
            new_user = User(
                email=email,
                fullname=fullname,
                google=google_id,
            ...
            db.session.commit()
            login_user(new_user)
            flash(f"Account created successfully, {new_user.fullname}!", "success")
...
def github_authorize():
    try:
        ...
        email = user_info.get("email")
        fullname = user_info.get("name")
        github_id = str(user_info.get("id"))  # unique GitHub ID

        # Haddii email ka GitHub aanu jirin, ka hel emails endpoint
        if not email:
...
def get_user_site_logo(user):
    ...
    # ---------------------------
    if user.role.value == "branch_admin" and branch_id:
    ...
    # ---------------------------
    if user.role.value == "school_admin" and school_id:
...
def profile():
    # ----- Role-based access -----
    allowed_roles = ['superadmin', 'school_admin','teacher', 'branch_admin']
    ...
    # ----- Inactive user check -----
    if current_user.status == 0:
        msg = "Your account is inactive. Please contact the superadmin." \
            if current_user.role.value in ['school_admin','teacher','branch_admin'] \
            else "Your account is inactive. Please contact the admin."
        flash(msg, "danger")
        return redirect(url_for('main.index'))
...
def remove_photo():
    ...
    if current_user.status == 0:
        if current_user.role.value in ['school_admin', 'teacher','branch_admin']:
            flash("Your account is inactive. Please contact the superadmin.", "danger")
        else:
...
def delete_all_students():
    ...
    if current_user.role.value != 'school_admin':
        flash("Ma haysatid ogolaansho aad ku tirtirto dhamaan ardayda.", "danger")
        return redirect(url_for('main.all_students'))
...
def delete_all_exam_hall_assignments():
    if current_user.role.value not in ["superadmin", "school_admin", "branch_admin"]:
        flash("❌ You are not authorized to delete exam hall assignments.", "danger")
...
def generate_exam_hall_assignments():
    ...
    if current_user.role.value not in ["superadmin", "school_admin", "branch_admin"]:
        flash("You are not authorized to generate exam hall assignments.", "danger")
        return redirect(url_for("main.dashboard"))
...
def generate_exam_tickets():
    if current_user.role.value not in ["superadmin", "school_admin", "branch_admin"]:
        flash("You are not authorized to generate exam tickets.", "danger")
        return redirect(url_for("main.dashboard"))
...
def delete_all_exam_tickets():
    ...
    if current_user.role.value not in ["superadmin", "school_admin", "branch_admin"]:
        flash("❌ You are not authorized to delete tickets.", "danger")
        return redirect(url_for("main.all_exam_tickets"))
...
def cumulative_results():
    ...
    if not active_year:
        ...
        return redirect(url_for('exam.all_exam_results'))
...
@login_required
def site_settings():
    # Get the first settings row, or create a default if none exists
    settings = SettingsData.query.first()
    if not settings:
    ...
    if form.validate_on_submit():
        try:
            ...
            upload_folder = "static/backend/uploads/settings"
            os.makedirs(upload_folder, exist_ok=True)
